...
exports.testEmail = https.onCall(async (data, context) => {
	// connect to the db and pull in all messages with a workspace ID
	let leads = []
	const leadsRef = admin
		.firestore()
		.collection("leads")
		.where("workspaceId", "==", "1")

	const leadsData = await leadsRef.get()
	leadsData.forEach((lead) => {
		leads.push({ leadId: lead.id, ...lead.data() })
	})
	console.log(leads)
	// pull all the workspace's leads

	// connect the messages to the workspace leads
